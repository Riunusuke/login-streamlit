...
def authenticate_user(
    email: str, password: str, require_email_verification: bool = False
...
def cookie_is_valid(cookie_manager: stx.CookieManager, cookie_name: str, tolerance_minutes: int = 5) -> bool:
    ...
    if (
        ...
        and token["exp_date"] > (datetime.now() + timedelta(minutes=tolerance_minutes)).timestamp()
...
def main() -> None:
    ...
    if not cookie_is_valid(cookie_manager, cookie_name) and not_logged_in(
        cookie_manager, cookie_name, preauthorized="hotmail.com"
    ):
        return None
